...
public class APIStepdDefs {
    String token;
    Response response;
    String emailGlobal;

    @Given("I logged Bookit api using {string} and {string}")
    // public void i_logged_Bookit_api_using_and(String string, String string)
    // we have to remained them to make them more dynamic, and then we will send a request as query parameters

    public void i_logged_Bookit_api_using_and(String email, String password){

        // this step is already simplify from out utilities class
    ...
    //for my second step definition we need to pass the token, but we need to retrieve the token from the first step
    public void i_get_the_current_user_information_from_api() {
        System.out.println("token = " + token);
        //after getting token we need to send a get request "/api/user/me" endpoint to get current user info
